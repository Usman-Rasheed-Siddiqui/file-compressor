...
def load_frequency_table(source, from_bytes=False):
    ...
    k = int.from_bytes(data[6:8], 'big')

    if k == 0 or k > 256:
        raise ValueError(f"Invalid symbol count k={k}")
    ...
    return freq_table, offset, payload_bytes, True, use_rle, use_delta
...
# bitbuffer helpers
def write_bits_to_bytes(bit_strings):
    """
    bit_strings: iterable of (bits_as_string) e.g. '1011' or you can pass integers and bit lengths.
    Returns bytes.
    """
    out = bytearray()
    buffer = 0
    ...
    for bits in bit_strings:
        # bits is a string like '101', iterate char-wise faster?
        for ch in bits:
            buffer = (buffer << 1) | (1 if ch == '1' else 0)
            bits_in_buffer += 1
            if bits_in_buffer == 8:
                out.append(buffer)
                buffer = 0
...
# better encode_data using code_map (code_map: {byte: '1010'})
def encode_data_to_bytes(data: bytes, code_map: dict):
    out = bytearray()
    buffer = 0
    bits_in_buffer = 0
    ...
    for b in data:
        ...
        for ch in bits:
            ...
            if bits_in_buffer == 8:
                # flush top 8 bits (from left). We keep left-aligned approach used above.
                out.append(buffer)
                buffer = 0
                bits_in_buffer = 0
